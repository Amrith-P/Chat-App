...
const AuthPage = () => {
  ...
  return (
    <div className="min-h-screen w-full flex flex-col md:flex-row bg-slate-950 text-white font-sans overflow-hidden">
      ...
      {/* RIGHT SECTION - Auth Form Card Container */}
      <div className="w-full md:w-1/2 bg-slate-950 p-6 md:p-12 lg:p-16 flex items-center justify-center relative overflow-y-auto custom-scrollbar">
        <div className="w-full max-w-md space-y-6">
          ...
          {/* Header Title */}
          <div className="text-center md:text-left space-y-1.5 transition-all duration-300">
          ...
          {/* Error Alert */}
          {error && (
            <div className="p-4 bg-red-500/10 border border-red-500/30 rounded-xl text-red-400 text-sm flex items-start justify-between animate-fadeIn">
              <span>{error}</span>
          ...
          {/* Success Alert */}
          {successMessage && (
            <div className="p-4 bg-emerald-500/10 border border-emerald-500/30 rounded-xl text-emerald-400 text-sm animate-fadeIn">
              {successMessage}
            </div>
          )}
            ...
            <div
              className={`transition-all duration-500 ease-in-out transform ${
                mode === 'login'
                  ...
                  : 'opacity-0 -translate-x-12 absolute inset-0 pointer-events-none z-0'
              }`}
            >
              <form onSubmit={handleLoginSubmit} className="space-y-4">
                ...
                <button
                  ...
                  className="w-full py-3.5 px-4 bg-emerald-500 hover:bg-emerald-400 disabled:opacity-50 text-slate-950 font-bold rounded-xl flex items-center justify-center space-x-2 transition-all duration-200 shadow-lg shadow-emerald-500/25 mt-2"
                >
                  <span>{isSubmitting ? 'Signing In...' : 'Sign In'}</span>
                  <FaSignInAlt />
                </button>

                {/* CREATE ACCOUNT LINK UNDER FORM */}
                <div className="text-center pt-5 border-t border-slate-800/80 text-xs text-slate-400 mt-4">
                  <span>Don't have an account? </span>
                  <button
                    type="button"
                    onClick={() => handleModeSwitch('register')}
                    className="text-emerald-400 font-bold hover:underline transition ml-1"
                  >
                    Create now
                  </button>
                </div>
            ...
            >
              <form onSubmit={handleForgotSubmit} className="space-y-4">
                ...
                <button
                  type="button"
                  onClick={() => handleModeSwitch('login')}
                  className="w-full py-2.5 text-sm text-slate-400 hover:text-white flex items-center justify-center space-x-2 pt-4"
                >
                  <FaArrowLeft />
