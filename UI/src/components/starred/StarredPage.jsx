...
import { FaStar, FaSearch, FaArrowRight, FaTrash } from 'react-icons/fa';
import { useStarred } from '../../context/StarredContext';

const StarredPage = () => {
  const { onJumpToChat } = useOutletContext() || {};
  const { starredMessages, unstarMessage } = useStarred();
  ...
  const filtered = (starredMessages || []).filter(
    (m) =>
      ...
      (m.chatName || '').toLowerCase().includes(searchTerm.toLowerCase())
  );
        ...
        {filtered.length === 0 ? (
          <div className="py-16 text-center text-slate-500 space-y-3">
            ...
            <p className="text-xs text-slate-600 max-w-xs mx-auto">
              Open any conversation, click the options menu on a message, and select "Star Message" to save important notes here.
            </p>
          </div>
        ) : (
          <div className="space-y-3 max-w-3xl">
            {filtered.map((msg) => {
              const displayName = (msg.isMe || msg.sender === 'You') ? 'You' : msg.sender;
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      <img
                        src={msg.chatAvatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(msg.chatName || 'User')}`}
                        alt={msg.chatName}
                      ...
                      <div>
                        <h4 className="font-bold text-xs text-white flex items-center space-x-2">
                          <span className={displayName === 'You' ? 'text-emerald-400 font-extrabold' : 'text-white'}>
                            {displayName}
                          </span>
